...
<div class="flex gap-x-[30px]">
    <div class="relative min-w-[255px] max-w-[255px]">
        <div class="fixed max-h-full min-w-[255px] max-w-[255px] space-y-6 pb-16 overflow-y-scroll">
            ...
            <div class="flex justify-between items-center space-x-4">
                ...
                <button type="button" on:click={()=>{enableDarkMode = !enableDarkMode}} use:inertia="{{ href: '/update-dark-mode', method: 'post', data: { dark_mode: enableDarkMode }, replace: true, preserveScroll: true }}" class="flex justify-center w-full bg-white text-[#186489] hover:text-[#798ef2] dark:bg-[#1E293B] p-6 rounded-[0.625rem] hover:shadow-lg">
                    {#if enableDarkMode}
    ...
    <div class="w-full space-y-6">
        <!--start nav-->
        <nav class="relative flex items-center justify-between hover:shadow-lg md:rounded-[0.625rem] px-4 py-4 bg-[#000000] text-13 text-[#F3F4FE]">
            ...
            <div class="dark:bg-slate-900 relative pointer-events-auto">
                <button on:click={()=>{showSearch = !showSearch; debounce('');}} type="button" class="hidden w-full lg:flex items-center text-sm leading-6 text-slate-400 border border-[#212020] rounded-md ring-1 ring-slate-900/10 shadow-sm py-1.5 pl-2 pr-3 hover:ring-slate-300 dark:bg-slate-800 dark:highlight-white/5 dark:hover:bg-slate-700">
        ...
        <main>
            <!--start filters-->
            <div class="flex justify-between text-[#186489] mb-2">
