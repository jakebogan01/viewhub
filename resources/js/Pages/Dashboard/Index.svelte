...
<div class="flex gap-x-[30px]">
    <div class="min-w-[255px] max-w-[255px] space-y-6">
        ...
        <aside class="relative flex-1 flex items-center md:items-end justify-between bg-green-500 h-[11.125rem] 1440:max-h-[8.5625rem] md:rounded-[0.625rem] md:h-auo px-6 py-6 md:px-8 md:py-6">
        ...
        <div class="bg-white dark:bg-[#1E293B] p-6 rounded-[0.625rem]">
            ...
            <div>
                ...
                {#each statuses as status, i (status.id)}
                    {#if status.count > 0}
                        <button type="button" use:inertia="{{ href: route, method: 'get', data: { search: filters.search, status: status.name, tag: filters.tag, sortby: filters.sortby, liked: filters.liked }, replace: true, preserveScroll: true }}" class="flex items-center justify-between w-full mt-4 1440:mt-6 group">
                            <span class="flex items-center">
                            ...
                            <span class="font-bold text-base group-hover:text-[#AD1FE9] dark:text-[#D1D7E9] {$page.url === `/dashboard?status=${status.name}` ? 'text-[#AD1FE9]' : 'text-[#647196]'}">{status.count}</span>
                        </button>
                    {/if}
                {/each}
            </div>
        ...
        <!--start settings-->
        <div class="flex justify-between items-center space-x-4 overflow-hidden">
            <button type="button" use:inertia={{ href: '/logout', method: 'post' }} class="flex justify-center w-full bg-white text-[#4661E6] hover:bg-gray-200 dark:bg-[#1E293B] p-6 rounded-[0.625rem]">
            ...
            <button type="button" on:click={toggleDarkMode} class="flex justify-center w-full bg-white text-[#4661E6] hover:bg-gray-200 dark:bg-[#1E293B] p-6 rounded-[0.625rem]">
            ...
            <a use:inertia href="/dashboard/settings" class="flex justify-center w-full bg-white text-[#4661E6] hover:bg-gray-200 dark:bg-[#1E293B] p-6 rounded-[0.625rem]">
