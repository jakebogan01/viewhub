...
<div class="flex flex-col lg:flex-row sm:px-6 1440:px-0 gap-x-[30px]">
    <div class="relative lg:min-w-[255px] lg:max-w-[255px]">
        <div class="flex lg:flex-col lg:fixed lg:max-h-full lg:min-w-[255px] lg:max-w-[255px] space-x-4 lg:space-x-0 lg:space-y-6 sm:pb-10 lg:pb-16 lg:overflow-y-scroll">
            ...
            <!--start tags-->
            <div class="hidden sm:block bg-white dark:bg-[#1E283A] px-3.5 py-6 hover:shadow-lg rounded-[0.625rem]">
                        ...
                        {#if status.count > 0}
                            <button type="button" use:inertia="{{ href: route, method: 'get', data: { search: filters.search, status: status.name, tag: filters.tag, sortby: filters.sortby, liked: filters.liked }, replace: true, preserveScroll: true }}" class="flex items-center justify-between w-full mt-2.5 group">
                                <span class="flex items-center">
                                    ...
                                    <span class="text-base group-hover:text-[#AD1FE9] dark:text-[#D1D7E9] dark:hover:text-[#4661E6] capitalize {$page.url === `/dashboard?status=${status.name}` ? 'text-[#AD1FE9]' : 'text-gray-500'}">{status.name}</span>
                                </span>
                                <span class="font-bold text-base group-hover:text-[#AD1FE9] dark:text-[#D1D7E9] {$page.url === `/dashboard?status=${status.name}` ? 'text-[#AD1FE9]' : 'text-[#21678B]'}">{status.count}</span>
                            </button>
                        {/if}
                    {/each}
                </div>
            ...
            <div class="hidden sm:flex flex-col lg:flex-row justify-between items-center space-y-1 lg:space-y-0 lg:space-x-4">
                <button type="button" use:inertia={{ href: '/logout', method: 'post' }} class="flex justify-center w-full bg-white text-[#186489] hover:text-[#798ef2] dark:bg-[#1E293B] p-6 rounded-[0.625rem] hover:shadow-lg">
                ...
                <button type="button" on:click={()=>{enableDarkMode = !enableDarkMode}} use:inertia="{{ href: '/update-dark-mode', method: 'post', data: { dark_mode: enableDarkMode }, replace: true, preserveScroll: true }}" class="hidden lg:flex justify-center w-full bg-white text-[#186489] hover:text-[#798ef2] dark:bg-[#1E293B] p-6 rounded-[0.625rem] hover:shadow-lg">
                    {#if enableDarkMode}
                ...
                <a use:inertia href="/dashboard/settings" class="flex justify-center w-full bg-white text-[#186489] hover:text-[#798ef2] dark:bg-[#1E293B] p-6 rounded-[0.625rem] hover:shadow-lg">
    ...
    <div class="w-full space-y-6">
        ...
        <nav class="relative flex items-center justify-between hover:shadow-lg sm:rounded-[0.625rem] px-4 py-4 bg-[#000000] text-13 text-[#F3F4FE]">
            ...
            <div class="dark:bg-slate-900 relative pointer-events-auto">
                <button on:click={()=>{showSearch = !showSearch; debounce('');}} type="button" class="w-full flex items-center text-sm leading-6 text-slate-400 border border-[#212020] rounded-md ring-1 ring-slate-900/10 shadow-sm py-1.5 pl-2 pr-3 hover:ring-slate-300 dark:bg-slate-800 dark:highlight-white/5 dark:hover:bg-slate-700">
        ...
        <main>
            <!--start filters-->
            <div class="flex justify-between text-[#186489] mb-2 px-6 sm:px-0">
                ...
                <div class="flex space-x-px">
                    ...
                    {:else}
                        <button type="button" class="block py-2 px-3 hover:bg-transparent rounded-md bg-white" aria-label="notifications"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" /></svg></button>
                    {/if}
                </div>
            </div>
            <!--end filters-->
