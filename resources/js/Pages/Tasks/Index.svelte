            ...
            <div class="relative mt-2">
                ...
                {#if showStatusDropdown}
                    <div class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
                        ...
                        <button type="button"
                                use:inertia="{{ href: '/', method: 'get', data: { search: filters.search, tag: filters.tag }, replace: true, }}"
                        ...
                        {#each statuses as status, x (status.id)}
                            <button type="button"
                                    use:inertia="{{ href: '/', method: 'get', data: { search: filters.search, status: status.name, tag: filters.tag}, replace: true, }}"
                                    class="font-normal block truncate text-gray-900 relative select-none py-2 pl-3 pr-9 w-full text-left"
        ...
        <div class="mt-4">
            <div class="bg-gray-300 p-6 min-w-[15.9375rem] max-w-[15.9375rem] rounded-[0.625rem]">
                <div class="flex flex-wrap justify-evenly gap-x-2 gap-y-3.5 text-13">
                    <button type="button" use:inertia="{{ href: '/', method: 'get', data: { search: filters.search, status: filters.status }, replace: true, }}" class="block bg-white hover:bg-gray-200 rounded-[0.625rem] px-4 py-1 font-semibold text-[#4661E6] cursor-pointer">All</button>

                    {#each tags as tag (tag.id)}
                        <button type="button" use:inertia="{{ href: '/', method: 'get', data: { search: filters.search, status: filters.status, tag: tag.name}, replace: true, }}" class="block bg-white hover:bg-gray-200 rounded-[0.625rem] px-4 py-1 font-semibold text-[#4661E6] cursor-pointer">{tag.name}</button>
                    {/each}
                </div>
            </div>
        </div>
